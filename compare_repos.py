...
GITHUB_ENTERPRISE_URL = os.getenv('GITHUB_ENTERPRISE_URL') # Required for GHE
...
def get_github_instance():
    """Authenticates and returns a PyGithub instance for GHE."""
    if not GITHUB_TOKEN:
        raise ValueError("GITHUB_TOKEN environment variable not set.")
    if not GITHUB_ENTERPRISE_URL:
        raise ValueError("GITHUB_ENTERPRISE_URL environment variable not set for GHE.")

    # Construct the API URL for GHE
    base_url = GITHUB_ENTERPRISE_URL.rstrip('/')
    api_url = f"{base_url}/api/v3"
    print(f"Connecting to GitHub Enterprise: {base_url}")
    return Github(base_url=api_url, login_or_token=GITHUB_TOKEN)
...
def main():
    ...
    for pair in org_pairs:
        ...
        # Use the original function name for organizations
        ...
        for prod_name in sorted(list(common_prod_names)):
            ...
            try:
                # Use the fetched organization objects
...
if __name__ == "__main__":
    ...
    if not GITHUB_TOKEN:
        ...
        exit(1)
    if not GITHUB_ENTERPRISE_URL:
         print("Error: GITHUB_ENTERPRISE_URL environment variable is not set.")
         print("Please set it in your .env file (e.g., GITHUB_ENTERPRISE_URL='https://github.yourcompany.com').")
         exit(1)
